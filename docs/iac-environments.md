...
The CI pipeline runs both phases plus the discovery on every apply-mode run; by hand the discovery is two CLI calls documented in the [infrastructure README](../infrastructure/README.md#apply-flow). Two load-bearing origin details: the origin `DomainName` must be the on.aws endpoint (it alone satisfies the ALB cert's SNI and host-header rule), with the AllViewerExceptHostHeader origin-request policy and CachingDisabled cache policy so login, Postgres-backed sessions, and WebSocket upgrades survive the proxy.
...
|---|---|---|
| Ships via | `docker-build.yml` → GHCR, docker-compose | The composed pipeline → ECR → ECS Express Mode |

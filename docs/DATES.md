...
For a **time-enabled layer**, `time.enabled` is the whole test. The URL is not inspected: core appends `datetime=` and `starttime=` to URLs that carry no placeholder, so a placeholder test would drop most of a mission's stack. Such a layer's cursor is its own `time.end` when `time.type` is `local`, and the Time Control's `time:getCurrent` otherwise; its window start is its own `time.start`, or `time:getStart`. A `local` layer the dashboard has not stamped a window onto yet follows the Time Control's cursor and start, which is what its features are filtered against until it does. The request the map made is window start to cursor, except in Point mode: the legend asks `time:getMode` once, and when it answers `'point'` the request has no start, for a `local` layer's stamped window as much as the Time Control's, since both came from the same Time Control. Any other answer, `null` included, leaves the window start as given. Then:

- **Core decides whether the layer has data at the cursor.** On every time step core's coverage gate tests the cursor against the layer's declared coverage (its Data Dates when it has them, else its Data Time Extent) and, when the cursor falls outside it, hides the layer and skips its request. The layer stays toggled on, so it keeps its row, but it paints nothing. The export legend reads that verdict from `layers:getDataCoverage`, and when it says `outOfDataRange`, the row reads `No data at cursor` and none of the rules below apply. A layer covering 2015 to 2016 with the cursor on 2024 is such a layer. When core has no verdict for the layer, or the request fails, the rules below decide.

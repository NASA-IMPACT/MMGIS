...
The three values enter the module as input variables (`express_internal_alb_arn`, `express_onaws_endpoint`, `express_alb_security_group_id`). The ALB ARN and the endpoint gate the CloudFront resources — while either is empty the module builds no distribution, VPC origin, or asset-bucket policy — and the security-group id separately gates the `:443` ingress rule; `iac-deploy.yml` only ever passes the three complete or not at all, so the half-built combinations never occur. Empty values are not a quiet default, though: the module refuses to plan with any of them missing unless it is also told `greenfield = true`, the explicit claim that this environment has never been fully built. CI sets that flag mechanically, after its pre-apply reads prove the environment is not fully built; a hand apply that forgets the values gets an error naming them instead of a plan that destroys CloudFront. The distribution itself additionally refuses destruction (`prevent_destroy`), so even a wrong greenfield claim against a live environment fails at plan time rather than deleting the front door (CI also treats the trio as all-or-nothing, so a partial read builds no CloudFront rather than a half-built one; only a hand apply that deliberately passes a partial trio under greenfield can drop the `:443` ingress rule — the flag's description warns against exactly that). Whoever runs Terraform is responsible for reading the values out of AWS and passing them in:
...
1. **Phase 1 — everything except CloudFront.** With the three variables empty and `greenfield = true`, the apply creates the cluster, the service (ECS provisions the internal ALB as part of it), the task definitions, RDS, the secret shells, ECR, the asset bucket, and the IAM roles.

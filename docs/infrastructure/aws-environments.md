...
One piece of the environment's own infrastructure is not Terraform's to create: the internal ALB. ECS provisions it itself when Terraform creates the Express Mode service, and the service resource never exposes the ALB's ARN as a Terraform attribute (the on.aws endpoint it does export, via the module's `express_ingress_paths` output). That is a problem for CloudFront, because two of its inputs are facts that do not exist until the service is running:
...
Nothing has to be done about the ALB's security group: Express Mode provisions it with `:80`/`:443` already open, so the VPC origin's ENIs reach the ALB without any rule from this module.

The two values enter the module as input variables (`express_internal_alb_arn` and `express_onaws_endpoint`). Together they gate the CloudFront resources — while either is empty the module builds no distribution, VPC origin, or asset-bucket policy — and `iac-deploy.yml` only ever passes them complete or not at all, so the half-built combinations never occur. Empty values are not a quiet default, though: the module refuses to plan with either missing unless it is also told `greenfield = true`, the explicit claim that this environment has never been fully built. CI sets that flag mechanically, after its pre-apply reads prove the environment is not fully built; a hand apply that forgets the values gets an error naming them instead of a plan that destroys CloudFront. The distribution itself additionally refuses destruction (`prevent_destroy`), so even a wrong greenfield claim against a live environment fails at plan time rather than deleting the front door. Whoever runs Terraform is responsible for reading the values out of AWS and passing them in:

- **In CI**, `iac-deploy.yml` reads them before every apply: `aws ecs describe-express-gateway-service` finds the service's newest revision, and `aws ecs describe-service-revisions` on that revision returns the load-balancer details — the ALB ARN and the on.aws endpoint.
- **By hand**, the same reads are the discovery step of the break-glass recipe in the [infrastructure README](../../infrastructure/README.md#hand-applies-break-glass).
...
1. **Phase 1 — everything except CloudFront.** With both variables empty and `greenfield = true`, the apply creates the cluster, the service (ECS provisions the internal ALB as part of it), the task definitions, RDS, the secret shells, ECR, the asset bucket, and the IAM roles.
...
3. **Phase 2 — CloudFront.** A second apply, now with the variables set, adds the CloudFront distribution, its VPC origin, and the asset-bucket policy.
...
- On a brand-new environment nothing is serving yet, so the first apply — under the same explicit `greenfield = true` — registers a placeholder `:latest` tag that does not exist in ECR (CI only pushes commit-SHA tags). Tasks crash-loop for a few minutes until the same run's app phase pushes a real image. Expected, and self-healing; without the flag, an empty image is an error, not a fallback.

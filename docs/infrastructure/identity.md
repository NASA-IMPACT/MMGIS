...
| `Resource: "*"` + exact action allowlist | CloudFront distributions / VPC origins / origin access controls (AWS-generated ids), security groups (AWS-generated ids; the VPC id is an uncommitted input), the Express Mode gateway's autoscaling alarms and log group (ECS names both; the boundary can pin only the account and region), `iam:CreateServiceLinkedRole` (scoped by service name instead), `ecs:RegisterTaskDefinition` / `ecs:DescribeTaskDefinition` / `ecs:DeregisterTaskDefinition` (no resource-level authorization), `ecr:GetAuthorizationToken`, and the `Describe*` read surface. The boundary backstops CI-created roles |
...
- **The database admin password's secret.** RDS creates it and names it `rds!db-<id>` — nothing environment-distinguishing in the name — so the runtime boundary scopes it by the `aws:rds:primaryDBInstanceArn` tag RDS sets, the only per-environment handle. Its encryption key is a bootstrap-created one (`alias/mmgis-master-secret`), shared by both environments, not the account's default Secrets Manager key: an AWS-managed key's policy delegates only metadata actions to IAM, so no grant on the apply role would let it create a database whose managed secret uses that key. The apply roles hold describe/encrypt/decrypt and `kms:CreateGrant` (conditioned on `kms:GrantIsForAWSResource`) on the shared key, the plan role may only describe it, and the runtime roles may only decrypt through Secrets Manager (`kms:ViaService`), which is the only way the password reaches a task.

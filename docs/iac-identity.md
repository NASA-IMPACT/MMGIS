...
- **Dashboard resources are environment-namespaced in lockstep with the app.** Dashboards are published by the application at runtime (a CloudFormation stack per dashboard) under the `mmgis-<env>-dashboard-` prefix; the IAM patterns pin to that exact string, and the [environments page](iac-environments.md#dashboard-stacks-app-created-environment-namespaced) covers the lockstep and the 11-character environment-name cap it forces.

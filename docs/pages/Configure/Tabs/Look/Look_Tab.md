...
Sets the background of the label that follows the cursor to a [CSS color](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value). That label shows feature hover names and tool messages such as measurements, so both take this color. Leave it unset to keep the theme's color. Error messages stay red whatever this is set to, and a tool asking for a specific color still gets it.

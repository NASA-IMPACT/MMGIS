...
**Update:** `POST /api/deployments/:id/update` re-bakes the bundle from the current mission config, calls CFN `UpdateStack` with the freshly rendered template — converging the stack's infrastructure to current values, including re-baking the current dashboards password into the auth Function — and polls to `UPDATE_COMPLETE`, then PutObjects the new assets to the existing bucket. An unchanged template comes back from CloudFormation as "no updates are to be performed" and skips the polling entirely, and a stack busy with another operation is waited out and the update retried. The CloudFront distribution is not replaced — same `deployment_id`, same stack, same URL. The row's `updatedAt` tracks the task's last activity on the row — a heartbeat — not the completion of a republish. The same ECS RunTask shape as Publish; the two part company only where no stack exists — Publish creates one, while Update creates one only for a row that never recorded a stack ARN (nothing was ever created) and otherwise refuses, since deleting the deployment and publishing again is what mints a new stack, and a new URL.
...
- its stack sits in a status only a delete can move on from, for any row status other than `deleting`/`deleted`, which are answered first — answered with the delete-and-republish guidance (and the warning that republishing mints a new URL), since no amount of waiting makes such a stack updatable;
- its stack could not be read at all, for the same set of row statuses — answered with the read error, since nothing then knows what the stack is doing;
- or the row is `provisioning`/`updating` and either its stack is mid-operation, or it was last touched within 90 minutes — twice the convergence deadline, and the publish task beats a heartbeat onto the row around each long step and from inside the asset copy and the bundle uploads, so the window only has to outlast one stack convergence — so a task may still be working for it. Past that window, a row that recorded a stack ARN whose stack is gone is refused with the delete-and-republish guidance; one that never recorded an ARN is let through, and the update creates the stack.

...
**Update:** `POST /api/deployments/:id/update` re-bakes the bundle from the current mission config, calls CFN `UpdateStack` with the freshly rendered template — converging the stack's infrastructure to current values, including re-baking the current dashboards password into the auth Function — and polls to `UPDATE_COMPLETE`, then PutObjects the new assets to the existing bucket. An unchanged template comes back from CloudFormation as "no updates are to be performed" and skips the polling entirely, and a stack busy with another operation is waited out and the update retried. The CloudFront distribution is not replaced — same `deployment_id`, same stack, same URL. The row's `updated_at` reflects the latest republish. The same ECS RunTask shape as Publish; the only difference is that Update converges the stack the row already owns, and refuses when that stack is gone unless the row never recorded a stack ARN (nothing was ever created, so an update creates it) — otherwise deleting the deployment and publishing again is what mints a new one (and a new URL). The endpoint refuses with a `409` before starting anything when the row is `deleting` or `deleted`; and, for a `provisioning`/`updating` row, when its stack could not be read at all, when its stack is mid-operation (with the delete-and-republish guidance for a status only a delete can move on from), when the row was last touched within the hour and a task may still be working for it, or when it recorded a stack ARN whose stack is gone.

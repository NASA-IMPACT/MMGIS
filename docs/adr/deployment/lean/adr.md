...
- Many **dashboards** — each frozen at publish time around one mission. Read-only frontend bundle behind a CloudFront distribution, optionally with edge-evaluated basic auth (a per-environment choice — constraint 8 below). Dashboards have no backend. A mission *is* a dashboard — the relationship is one-to-one. To publish a variation, copy the mission config into a new mission and publish that.
...
5. **Dashboard URLs are bare CloudFront distribution names.** Each dashboard is reachable at its distribution's default domain name (`d<n>.cloudfront.net`). The admin records this URL in its registry and returns it to the publisher. A friendly hostname cannot be pointed straight at the dashboard's distribution: CloudFront serves an aliased hostname only when the distribution carries that name in `Aliases` and an ACM certificate covering it, and lean provisions neither. The supported route is the owner's own CloudFront distribution serving the dashboard under a path prefix (see `docs/infrastructure/serving-a-dashboard-from-your-domain.md`); the admin does not manage DNS or certificates for dashboards. The bundle and uploaded assets are referenced document-relative (`assets/…`, no leading slash), so that hostname change needs no rebake.
...
8. **Dashboard auth is one shared password across all dashboards, per environment.** A single value, edge-evaluated by a CloudFront Function. Whether an environment gates its dashboards at all is that environment's Terraform choice (`dashboards_require_auth`); development gates, production publishes dashboards with no password. Per-dashboard passwords are out of scope until a production audience needs it.
...
- **One CloudFront Function** attached to the viewer-request event. JavaScript that checks the `Authorization: Basic` header against a baked password and returns `401` on mismatch (the password value is the same across every dashboard), then validates and strips the `X-Forwarded-Prefix` prefix from the request URI and redirects the slash-less entry URL to its trailing-slash form. The Function ships in every stack; a boolean baked into it at publish decides whether the password check runs, and an environment that sets `dashboards_require_auth = false` bakes it off. The prefix handling is what the Function always carries.
- **No Route 53 record, no `Aliases`, no ACM certificate.** The dashboard is reachable at its CloudFront default name. A friendly hostname needs `Aliases` plus a matching ACM certificate on the distribution, which this stack does not declare; an owner who wants one fronts the dashboard with their own CloudFront distribution under a path prefix, as a separate, out-of-MMGIS step.
...
4. Task calls CFN `CreateStack` with a rendered template (bucket + distribution + Function, password baked into the Function source unless the environment sets `dashboards_require_auth = false`, in which case the dashboard publishes without one). Stack name encodes the dashboard ID for idempotency.
...
**Update:** `POST /api/deployments/:id/update` re-bakes the bundle from the current mission config, calls CFN `UpdateStack` with the freshly rendered template — converging the stack's infrastructure to current values, including re-rendering the auth Function under the environment's current gate setting — and polls to `UPDATE_COMPLETE`, then PutObjects the new assets to the existing bucket. The CloudFront distribution is not replaced — same `deployment_id`, same stack, same URL. The row's `updated_at` reflects the latest republish. The same ECS RunTask shape as Publish, with `UpdateStack` + polling in place of `CreateStack` + polling.

...
**Update:** `POST /api/deployments/:id/update` re-bakes the bundle from the current mission config, calls CFN `UpdateStack` with the freshly rendered template — converging the stack's infrastructure to current values, including re-baking the current dashboards password into the auth Function — and polls to `UPDATE_COMPLETE`, then PutObjects the new assets to the existing bucket. An unchanged template comes back from CloudFormation as "no updates are to be performed" and skips the polling entirely, and a stack busy with another operation is waited out and the update retried. The CloudFront distribution is not replaced — same `deployment_id`, same stack, same URL. The row's `updated_at` reflects the latest republish. The same ECS RunTask shape as Publish; the only difference is that Update requires the stack to already exist, where Publish creates one when it does not.

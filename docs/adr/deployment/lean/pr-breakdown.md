This is an LLM artifact. It slices the lean (keep) implementation work into reviewable pull requests. Treat it as a draft sequencing aid, not a contract.

# Lean (keep) — PR breakdown
...
### PR 1 — Deployment-mode foundation (Phase 1) <a id="pr-1"></a>
- `API/utils/deploymentMode.js` + `src/pre/deploymentMode.js` helpers (`isLean`/`isFull`, defaults to `full`, throws on unknown).
- `SERVER`/`STATIC_*` build flags, env allowlist, gitignored baked-config stub.
- No behavior change. **Blocks: everything.**

### PR 2 — Gate sidecar proxy (Phase 2) <a id="pr-2"></a>
- `adjacent-servers` proxy + spawner no-op in lean; `WITH_*` forced false at the Pug shell.
- `init-db.js` skips `mmgis-stac` DB in lean.
- _Depends: PR 1._

### PR 3 — Gate Datasets & Geodatasets + Configure mode flag (Phase 3 core) <a id="pr-3"></a>
...
### PR 4 — Gate Draw (Phase 3, Draw portion) <a id="pr-4"></a>
...
### PR 5 — Gate Missions middleware, `_time_` compositor & link shortener (Phases 4 + 5) <a id="pr-5"></a>
- Gate the three `Missions/` mounts and the `sharp` `_time_` compositor in lean (`full` unchanged).
- Gate the link-shortener route mount.
- _Depends: PR 1._

### PR 6 — Configure SPA lean polish (Phase 3, "other surfaces") <a id="pr-6"></a>
...
### PR 7 — Static frontend: SERVER flag + dispatcher + ServiceUrls static mode (Phase 6 core) <a id="pr-7"></a>
...
### PR 8 — Publish flow: backend + Deployments Configure page (Phase 7) <a id="pr-8"></a>
...
### PR 9 — Static-mode COG range, projection WKT & histogram disable (Phase 6) <a id="pr-9"></a>
...
### PR 10 — S3 asset upload repoint (Phase 4, upload portion) — **BLOCKED on #103** <a id="pr-10"></a>
- Swap `Upload/uploadRouter.js` storage from local `Missions/` to S3 PutObject in lean; return a root-relative `/assets/…` path.
- _Depends: PR 5, the asset bucket (PR 11), and #103 merging to `development`._

### PR 11 — AWS infra: ECS task defs, IAM, CloudFront, GHA deploy (Phase 8) <a id="pr-11"></a>
...
### PR 12 — Hardening (Phase 9) <a id="pr-12"></a>
...
### PR 13 — Cleanup + dual-mode CI (Phase 10) <a id="pr-13"></a>
...
- **PR 6** after PR 3. **PR 8** after PR 3 + PR 7. **PR 9** after PR 7. **PR 11** after PR 8. **PR 10** after PR 5 + PR 11 + #103.
- **PR 13** last.

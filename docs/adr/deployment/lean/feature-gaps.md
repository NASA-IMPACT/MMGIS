...
| **Card** | Uploaded images resolve to `Missions/<mission>/…` → 404 in a dashboard | handled by the S3 upload repoint (upload returns a document-relative `assets/<mission>/<subdir>/uploads/<file>` key, which `resolveMissionAssetUrl` in `src/pre/uploadKey.ts` recognizes by that shape and resolves relative to the document base instead of mission-prefixing it) plus the publish-time asset copy into the dashboard bucket |

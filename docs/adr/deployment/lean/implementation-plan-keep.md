...
> **Post-merge note:** `InterpolateHtmlPlugin` is already configured in `configuration/webpack.config.js` (it substitutes `%NODE_ENV%` in `public/index.html`) — reuse it for the `SERVER` substitution in Phase 6 rather than adding a plugin. The build now also produces a top-level `dist/` of theme assets via `npm run build:themes`, and `webpack.config.js` was reverted toward upstream — start Phase-1 webpack edits from that merged baseline. React is now 19.x with USWDS/TypeScript/Sass added; the deployment-mode helpers must coexist with these.
...
- **COG fields in the layer modal.** `configure/src/metaconfigs/layer-tile-config.json`, `layer-data-config.json`, `layer-image-config.json` declare COG-related fields. The populate-from-cog/info button (in `configure/src/core/Maker.js`) calls `/api/utils/getbands` / `/titiler/cog/info`, which are dropped in lean. Hide the button in lean mode (it's a Maker render branch keyed on the field type). The fields themselves can stay since externally hosted COGs work — the gating is just about the populate-helper, not the layer type.
  - **Post-merge note:** the merge added an **"External Service URLs"** section and a `titiler-url` sourceType to `layer-tile-config.json` (#63). **Keep these visible in lean** — they are the mechanism missions use to point layers at external sidecars, which is exactly what lean needs. Only the populate-helper is hidden. Concrete anchors: the inline same-origin call is at `configure/src/core/Maker.js:1854` (`${origin}/titiler/cog/info?url=...`); the button's result copy is at `Maker.js:579`/`:587`. (Grep those line anchors, not "Populate from cog" — that literal string is not in the source.)
...
> **Post-merge note — most of the "centralize sidecar URLs" work is already done (#63).** `src/essence/Basics/ServiceUrls/ServiceUrls.js` exists and implements the resolver this phase was going to create: per-layer field → global `mmgisglobal.options.services` → local `/<service>` default, for all five sidecars. The four files this phase named are **already migrated** to it (`Map_.js`, `Layers_.js`, `IdentifierTool.js`, `LayersTool.js`). So the remaining Phase-6 service-URL work is *extend-and-adapt*, not build-from-scratch:
> 1. **Add TiPG / Veloserver URL builders.** `getTipgUrl`/`getVeloserverUrl` getters exist (`ServiceUrls.js:80-81`) but have no `build*Url` companions — TiTiler/TiTiler-pgSTAC/STAC do.
> 2. **Static-mode resolution.** In `static` builds `ServiceUrls` must return baked absolute URLs with **no same-origin fallback** (the default `/<service>` paths 404 in a backend-less dashboard). Gate on `mmgisglobal.SERVER !== 'node'`.
> 3. **`transformStacUrl` is already migrated (correction).** `src/essence/Basics/Layers_/Layers_.js:322` already builds its base via `ServiceUrls.getTiTilerPgStacUrl(layerData)`, so the "Additional frontend behavior" fix for it below — and the Animation-tool path that calls it — is **done by #63**. Nothing to do beyond item 2's static-mode behavior. (The original bullet further down is annotated accordingly.)
> 4. **LayerManager bypasses `ServiceUrls`.** `src/essence/Tools/LayerManager/lib/utils/titiler.ts:1` (`getTiTilerBaseUrl`) builds same-origin `${origin}${pathname}/titiler`. New surface this phase must handle in static mode: route through `ServiceUrls`/baked URL, or skip COG colormap fetches when `SERVER==='static'`.
>
> The dispatcher core is intact and unchanged by the merge: the dormant `if (window.mmgisglobal.SERVER != 'node')` branch is at `src/pre/calls.js:169`, and `c` has **40** entries (`calls.js:4-162`). Don't be fooled by a naïve grep returning 41 — `api: api` at `calls.js:214` is an `export default` member, not a call entry.
...
- ~~New: `src/essence/Basics/serviceUrls.js`~~ **Already exists as `src/essence/Basics/ServiceUrls/ServiceUrls.js` (#63).** Extend it per the post-merge note above (TiPG/Veloserver builders; static-mode no-fallback resolution). Server-mode already returns same-origin paths so the full admin still hits `/titiler`.
- ~~Edit the four URL-builder files~~ **Already migrated to `ServiceUrls` by #63:** `src/essence/Basics/Map_/Map_.js`, `src/essence/Basics/Layers_/Layers_.js`, `src/essence/Tools/Identifier/IdentifierTool.js`, `src/essence/Tools/Layers/LayersTool.js`. No work here beyond the static-mode behavior of `ServiceUrls` itself.
...
- **Fix `transformStacUrl` to honor the layer's actual URL.** **✓ Done by #63** — `transformStacUrl` (`Layers_/Layers_.js:322`) already resolves via `ServiceUrls.getTiTilerPgStacUrl(layerData)`; the Animation path that calls it inherits the fix. Only the static-mode no-fallback behavior of `ServiceUrls` remains. Original description retained for context: the Animation tool's offscreen renderer calls `transformStacUrl(...)` which *previously* built same-origin `/titilerpgstac/...` paths from `window.location`; the regular layer renderers already honored external URLs, but the animation pathway did not.
...
> **Post-merge note — theme assets.** The merge added a `dist/` theme-asset step: `npm run build:themes` (`scripts/build-themes.sh`) emits `dist/{theme}.css` + fonts/images, and `scripts/build.js` copies `dist/` → `build/dist/`. Both the admin image build **and** the static dashboard publish build (Phase 7's `scripts/publish-static.js`) must run `build:themes` and bake `dist/` into their output, or themed missions ship without CSS/fonts. Account for `build:themes` in the Dockerfile/CI build steps and in the publish task.

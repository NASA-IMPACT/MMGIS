...
// Static checks on the lean deployment's AWS recipes in infrastructure/:
// every JSON file must parse, the IAM must stay least-privilege (no
// `Resource: "*"` except the unscopeable ecr:GetAuthorizationToken, dashboard
// grants pinned to the mmgis-dashboard-* prefix, PassRole for both publish
// roles), the task definitions must carry every env var the publish flow's
// code actually reads (publish-only vars like MMGIS_DASHBOARDS_PASSWORD on
// the publish task, the rest on the admin task), and the Terraform module
// must stay in lockstep with the names the app composes.

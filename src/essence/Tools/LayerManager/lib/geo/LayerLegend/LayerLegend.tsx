...
export function LayerLegend({
    ...
    const leadStep = parseStep(forecast?.leadStep) ?? parseStep(DEFAULT_LEAD_STEP)!
    const leadIso = forecast?.leadStep ?? DEFAULT_LEAD_STEP
    const runAt = parseUtc(selectedRun)
    const clockAt = parseUtc(selectedTime)
    const leadLabel =
        runAt && clockAt ? formatLead(stepsBetween(runAt, clockAt, leadStep), leadStep, leadIso) : null
                        ...
                        >
                            {forecast.runs.map((run, index) => (
                                <option key={run.datetime} value={run.datetime}>
                                    {`${formatRun(run.datetime, leadStep)} · ${

...
var LayersTool = {
    ...
    populateCogScale: function (layerName) {
        ...
        // Marks this as a derived snapshot, not an authored legend, so the
        // shared legend model (buildLayerLegendData) can tell it apart from
        // variables.legend/CSV-authored ones.
        L_.layers.data[layer.name]._legendAutoGenerated = true
...
*/
    ...
    $('#layersTool').on('click', '.setGlobalTimeFromExtent', function (e) {
        ...
        const dataStart = resolveTimePolicy(layerData?.time?.dataStartTime)
        const dataEnd = resolveTimePolicy(layerData?.time?.dataEndTime)

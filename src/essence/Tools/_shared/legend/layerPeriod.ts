...
export const layerPeriodFor = (
    ...
    // Every step is measured from the anchor rather than from the previous
    // step, so month arithmetic cannot accumulate the end-of-month clamp (a
    // Jan 31 anchor stepped by P1M twice reaches Mar 31, not Mar 28).
    const anchorDate = new Date(anchorMs)
    let steps = 0
    let end = addDuration(anchorDate, duration, 1)
    while (end.getTime() <= cursorMs) {
        steps += 1
        if (steps > MAX_STEPS) return null
        end = addDuration(anchorDate, duration, steps + 1)
    }
    return {
        start: addDuration(anchorDate, duration, steps).toISOString(),
        end: end.toISOString(),
    }
}

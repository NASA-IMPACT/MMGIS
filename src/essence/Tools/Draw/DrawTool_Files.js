...
var Files = {
    ...
    populateFiles: function (activeFileId) {
        ...
        $(
            ...
            $('#cmExportGo').on('click', () => {
                ...
                DrawTool.getFile(body, function (fileResponse) {
                    let geojson = fileResponse.geojson
                    let filename = ''
                    if (isPub) {
                        filename = 'CAMP_Latest_Map'
                        geojson._metadata = fileResponse.file
                    } else {
                        filename =
                            fileResponse.file[0].file_name +
                            '_' +
                            fileResponse.file[0].id +
                            '_' +
                            fileResponse.file[0].file_owner
                        geojson._metadata = [fileResponse.file[0]]
                    }
                    ...
                    switch (format) {
                        ...
                        case 'kml':
                            ...
                            if (fileResponse?.file?.[0]?.template?.template) {
                                fileResponse.file[0].template.template.forEach(
                                    (f) => {
                                        if (
                                            f.type === 'date' &&
                                            f.isEnd === true
        ...
        $('.drawToolDrawFilesListElem').on('mouseenter', function () {
            ...
            var fileId = parseInt($(this).attr('file_id'))
            var fileLayers = L_.layers.layer['DrawTool_' + fileId]
            if (!fileLayers) return
            for (var i = 0; i < fileLayers.length; i++) {
                if (
                    fileLayers[i] != null &&
                    fileLayers[i].temporallyHidden != true
                ) {
                    // Skip associated points - they shouldn't be highlighted on file hover
                    if (fileLayers[i]._isAssociatedPoint === true) continue

                    if (typeof fileLayers[i].setStyle === 'function')
                        fileLayers[i].setStyle({ color: '#7fff00' })
                    else if (fileLayers[i].hasOwnProperty('_layers')) {
                        //Arrow
                        var layers = fileLayers[i]._layers
        ...
        $('.drawToolDrawFilesListElem').on('mouseleave', function () {
            ...
            for (var i = 0; i < fileLayers.length; i++) {
                var style
                if (fileLayers[i] != null) {
                    // Skip associated points - they don't have feature.properties.style
                    if (fileLayers[i]._isAssociatedPoint === true) continue

                    if (
                        !fileLayers[i].hasOwnProperty('feature') &&
                        fileLayers[i].hasOwnProperty('_layers')
                    )
                        style =
                            fileLayers[i]._layers[
                                Object.keys(fileLayers[i]._layers)[0]
                            ].feature.properties.style
                    else style = fileLayers[i].feature.properties.style
                    ...
                    if (Map_.activeLayer) {
                        if (
                            typeof fileLayers[i].setStyle === 'function' &&
                            ((fileLayers[i].hasOwnProperty('_layers') &&
                                fileLayers[i].hasLayer(Map_.activeLayer)) ||
                                Map_.activeLayer === fileLayers[i])
                        ) {
                            color =
                                (L_.configData.look &&
                    ...
                    if (typeof fileLayers[i].setStyle === 'function')
                        fileLayers[i].setStyle({ ...style, color })
                    else if (fileLayers[i].hasOwnProperty('_layers')) {
                        //Arrow
                        var layers = fileLayers[i]._layers
                        if (fileLayers[i].isLinework) {
                    ...
                    if (Map_.activeLayer === fileLayers[i]) {
    ...
    refreshFile: function (
        ...
        if (forceGeoJSON) {
            ...
            DrawTool.getFile(
                body,
                (function (fileId, selectedFeatureIds) {
        ...
        function renderFileFeatures(
            ...
            for (var i = 0; i < features.length; i++) {
                ...
                Files.renderAssociatedPoints(features[i], fileId, layerId)
            }
            ...
            let indexOf = DrawTool.labelsOn.indexOf(fileId + '')
            if (indexOf != -1) {
                DrawTool.labelsOn.splice(indexOf, 1)
                DrawTool.toggleLabels(fileId + '')
            }
    ...
    reloadFileInExtent: function(
        ...
        DrawTool.getFile(body, function(data) {
            ...
            // Don't clear features here - let refreshFile's renderFileFeatures handle it
            ...
            // refreshFile with forceGeoJSON will go through renderFileFeatures which clears and re-renders
    ...
    toggleFile: function (
        ...
        var resolvedFileId = parseInt($(this).attr('file_id'))
        if (argumented) resolvedFileId = fileId

        var layerId = 'DrawTool_' + resolvedFileId

        if (
            forceToggle == 'off' ||
            (forceToggle != 'on' &&
                DrawTool.filesOn.indexOf(resolvedFileId) != -1)
        ) {
            //OFF
            if (DrawTool.currentFileId == resolvedFileId) {
                $(`.drawToolFileSelector[file_id=${resolvedFileId}]`).click()
            }

            DrawTool.filesOn = DrawTool.filesOn.filter(function (v) {
                return v !== resolvedFileId
            })
            ...
            if (!argumented) {
                ...
                //Change icon
                $(
                    `.drawToolFileCheckbox[file_id=${resolvedFileId}]`
                ).removeClass('on')
                $(`.drawToolFileCheckbox[file_id=${resolvedFileId}]`)
                    .parent()
                    .parent()
            ...
            if (DrawTool.dynamicExtent && DrawTool.dynamicExtent.truncated) {
                delete DrawTool.dynamicExtent.truncated[resolvedFileId]
            }
            ...
            if (forcePopulateShapes) {
                $(
                    `.drawToolContextMenuHeaderClose[file_id=${resolvedFileId}]`
                ).click()
                DrawTool.populateShapes()
            }
        } else {
            //ON
            DrawTool.filesOn.push(resolvedFileId)

            if (!argumented) {
                //Change icon
                $(`.drawToolFileCheckbox[file_id=${resolvedFileId}]`).addClass(
                    'on'
                )
                $(`.drawToolFileCheckbox[file_id=${resolvedFileId}]`)
                    .parent()
        ...
        DrawTool.lastToggledFileId = resolvedFileId
    },
    toggleLabels: function (file_id) {
        var fileLayers = L_.layers.layer['DrawTool_' + file_id]
            ...
            for (var i = 0; i < fileLayers.length; i++) {
                if (fileLayers[i] != null) {
                    if (fileLayers[i]._layers) {
                        ...
                        fileLayers[i].feature.properties.annotation != true
                    ) {
                        var p = fileLayers[i]
                        if (isOn) p.closePopup()
                        else p.openPopup()

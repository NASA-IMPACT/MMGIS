...
/**
 * Build the GeoJSON Feature emitted by `drawcomplete` for a given drawing
 * shape and vertex list. Returns `null` if the vertices don't form a valid
 * shape (e.g. polygon with fewer than 3 vertices).
 *
 * Polygon: closes the ring (appends first vertex if missing).
 * Rectangle: 2 corner vertices → axis-aligned 4-vertex polygon.
 * Circle: vertex 0 = center, vertex 1 = edge → 32-segment polygon approximation.
 */
export function buildDrawnFeature(
    ...
    if (shape === 'polygon') {
        ...
        if (first[0] !== last[0] || first[1] !== last[1]) ring.push(first.slice() as number[])
        return {
            type: 'Feature',
            properties: { source: 'draw', shape: 'polygon' },
    ...
    if (shape === 'rectangle') {
        ...
        return {
            type: 'Feature',
            properties: { source: 'draw', shape: 'rectangle' },
            geometry: {
                type: 'Polygon',
                coordinates: [[
                    [w, s],
    ...
    if (shape === 'circle') {
        ...
        return {
            ...
            geometry: {
                type: 'Polygon',
                coordinates: [approximateCircleRing(center.lat, center.lng, radius)],
            },
        }
    }
    return null

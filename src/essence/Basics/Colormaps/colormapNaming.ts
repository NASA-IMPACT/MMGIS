...
// ramp, `viridis_r` the reversed one. Both the raster renderer
...
export const getBaseColormapName = (name: string | null | undefined): string => {
    if (!name) return ''
    return name.trim().replace(REVERSED_SUFFIX, '')
}

/**
 * Case-insensitively matches a colormap name (optionally `_r`-suffixed)
 * against a set of canonical keys, returning the key in its original casing.
 * The bundled js-colormaps evaluator keys its ramps by their published casing
...
export const findColormapKey = (
    ...
    if (!base) return null
    return keys.find((key) => key.toLowerCase() === base) ?? null
}

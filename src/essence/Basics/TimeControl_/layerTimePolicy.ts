...
export function addDuration(date: Date, d: Duration, factor: number): Date {
    ...
    // Day-of-month is clamped to the target month's length, so Jan 31 + P1M
    // is Feb 28 and not Mar 3, which setUTCMonth alone would overflow to.
    ...
    out.setUTCDate(Math.min(day, monthEnd) + factor * (d.days + 7 * d.weeks))

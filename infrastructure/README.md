...
- **Secret values**, filled in after the apply — generated by CI for the four
  passphrase slots, hand-set for the Mapbox token
  ([below](#where-the-secret-values-come-from)). Terraform defines the secrets'
...
Terraform created empty secret shells; the values are filled in afterwards, and
nothing that follows touches Terraform state. **Nothing DB-related is set by
hand or by CI.** The database password exists in exactly one place — the
RDS-managed master secret — and the containers reference that secret's
`password` key directly at task start; host, port, user, and database name are
not secrets and ride as plain environment values on both task definitions.

**The CI path (normal).** The infrastructure engine runs a secret-bootstrap
step immediately after the phase-1 apply. For each of the four MMGIS-generated
slots it checks the shell's metadata and, if the shell is **empty**, generates a
word passphrase and stores it:

| Secret | Generated |
| --- | --- |
| `mmgis/<env>/session-secret` | 8 words (~103 bits) |
...
behind. CI can only describe and put; it deliberately has no permission to read
a secret value back, so the values live only in Secrets Manager and in the
...
The same command sets any of the five slots. `mmgis/<env>/mapbox-token` is the
one that always needs it — an external credential CI can never invent,
injected as `MAPBOX_TOKEN` on the admin task only.
...
- **Serving a published dashboard from a customer's own domain.** When a team
  ...
  existing dashboard picks it up via a republish with either action.
...
The same first run also generates the four passphrase secrets, immediately
after phase 1: the shells it just created are empty, so the bootstrap step
fills them. Tasks that tried to launch in the window before the values landed
fail on the missing `valueFrom` and self-heal through the same ECS launch
retries. A fresh environment therefore converges with no human input at all
except the Mapbox token.

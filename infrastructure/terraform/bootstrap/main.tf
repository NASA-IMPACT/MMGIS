...
locals {
  ...
  # GitHub Environment names must match these exactly: the OIDC subject of an
  # environment-bound job is repo:<owner/name>:environment:<env>, and renaming
  # an Environment breaks every AssumeRoleWithWebIdentity here.
  environments = toset(["development", "production"])

  # Bucket names are S3-global, so they carry the (never-committed) account id.
  ...
  # The audience is always sts.amazonaws.com for GitHub's provider; the
  # subject is what actually pins who may assume what.
